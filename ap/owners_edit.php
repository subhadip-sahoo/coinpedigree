...
$ls_page_parent = "owners_listing.php";
...
if (isset( $lp_delete ) == true ) {
	...
	$result_delete = $conn->prepare("delete from owners where id_owner= :id_owner");
	$result_delete->execute(array(":id_owner"=>$lp_id));
...
if (isset( $lp_save ) == true ) {
	...
	if ($lp_id != "new") {
		...
		$result = $conn->prepare("select * from owners where id_owner = :id_owner");
		$result->execute(array(":id_owner"=>$lp_id));
